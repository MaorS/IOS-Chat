...
class MSActionSheet: UIAlertController,  UIImagePickerControllerDelegate, UINavigationControllerDelegate  {
    ...
    func showFullActionSheet(on vc : UIViewController,handler : @escaping () -> Void) {
        let sheet = create()
        sheet.addLibrary().addFrontCamera().addRearCamera().addVideos().addCancelButton().show(on: vc)
    ...
    //
    func addVideos(title : String = "Videos", destructive style : Bool = false , image : UIImage? = nil) -> MSActionSheet{
        
        self.videos = title
        let videoAction = UIAlertAction(title: videos, style: isDestructive(style), handler: handler)
        
        if let img : UIImage = image{ // client select different image
            videoAction.image = img
        }else{
            if let img = UIImage(named: "msactionsheet_rearCam") { //the image exists..
                videoAction.image = img
            }
        }
        MSActionSheet.sheet?.addAction(videoAction)
        return self
    }
    ...
    /// - parameters:
    ///   - vc: The view controller to show the ActionSheet at
    func show(on vc : UIViewController, onShow : (() -> Void)? = nil){
        clientVC = vc
        vc.present(MSActionSheet.sheet!, animated: true, completion: onShow)
    }
    ...
    func getResizedImage(toSize newSize: CGSize) -> UIImage? {
    ...
    /// Get the selected video by the user
    /// (Make sure this method called inside the onFinishPicking() method or inside showFullActionSheet() handler)
    ...
    /// - parameters:
    ///   - handler: The function that  will execute after the user took the image
    func onFinishPicking(handler : @escaping () -> Swift.Void){
